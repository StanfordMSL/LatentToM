...
def real_data_to_replay_buffer_decentralized(
        dataset_path: str, 
    ...
    for key, value in in_replay_buffer.data.items():
        chunks_map[key] = value.shape                # get shape of each dataset
        compressor_map[key] = lowdim_compressor      # assigns compression settings
    ...
    n_steps = in_replay_buffer.n_steps     # n_steps = 27672, the total number of recorded time steps
    """
    episode_ends - an array containing the last time step index of each episode
    episode_lengths - contains the number of time steps in each episode
    episode_starts - the first time step index of each episode.
    """
    episode_starts = in_replay_buffer.episode_ends[:] - in_replay_buffer.episode_lengths[:]
    # Stores the number of frames per episode
    episode_lengths = in_replay_buffer.episode_lengths
    # Retrieves timestamps for all frames, its shape is (27672,)
    timestamps = in_replay_buffer['timestamp'][:]
    # dt is 0.09999990463256836
