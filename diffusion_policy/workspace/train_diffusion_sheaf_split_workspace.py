...
from diffusion_policy.policy.diffusion_sheaf_split_policy import DiffusionSheafSplitPolicy
from diffusion_policy.dataset.base_dataset import BaseImageDataset
from diffusion_policy.env_runner.base_image_runner import BaseImageRunner
from diffusion_policy.common.checkpoint_util import TopKCheckpointManager
from diffusion_policy.common.json_logger import JsonLogger
from diffusion_policy.common.pytorch_util import dict_apply, optimizer_to, build_arm_sub_batch
...
class TrainDiffusionSheafSplitWorkspace(BaseWorkspace):
    ...
    def __init__(self, cfg: OmegaConf, output_dir=None):
        ...
        self.arm1_model: DiffusionSheafSplitPolicy = hydra.utils.instantiate(cfg.policy)
        self.arm2_model: DiffusionSheafSplitPolicy = hydra.utils.instantiate(cfg.policy)

        self.arm1_ema_model: DiffusionSheafSplitPolicy = None
        self.arm2_ema_model: DiffusionSheafSplitPolicy = None
    ...
    def compute_confidence_one_way_loss(self, embedding_arm1, embedding_arm2, confidence_arm1, confidence_arm2):
        """
        embedding_arm1, embedding_arm2: [B, D]
        confidence_arm1, confidence_arm2: [B, 1] ∈ [0,1]
        """
        lambda_entropy = 0.01
    ...
    def compute_sheaf_loss(self, shared_arm1, shared_arm2, private_arm1, private_arm2, model_arm1=None, model_arm2=None):
        """
        shared_arm1, shared_arm2: [B, 512]
        private_arm1, private_arm2: [B, 519] image + pose
        model_arm1, model_arm2: DiffusionSheafSplitPolicy
        """
        alpha = 1.0
        beta = 0.5
        gamma = 0.5
            ...
            for local_epoch_idx in range(cfg.training.num_epochs):
                ...
                if (self.epoch % cfg.training.sample_every) == 0:
                    with torch.no_grad():
                        ...
                        """
                        Sheaf Laplacian Online Adjustment or not?
                        """
...
def main(cfg):
    workspace = TrainDiffusionSheafSplitWorkspace(cfg)
    workspace.run()
